...
# Opening the JSON file and load data
...
# Creating the 'imdb_movies' database if it doesn't exist
create_db_query = "CREATE DATABASE IF NOT EXISTS imdb_movies;"
cursor.execute(create_db_query)
conn.commit()

# Using the 'imdb_movies' database
db_config["database"] = "imdb_movies"
conn = mysql.connector.connect(**db_config)
cursor = conn.cursor()

# Creating the 'movies' table with the 'genres' column as VARCHAR
create_table_query = """
...
# Committing changes and close the connection
conn.commit()
conn.close()
